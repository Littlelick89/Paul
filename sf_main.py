"""Salesforce Uploader – GUI

Reads the Excel file produced by the OCR program and uploads each row
as a new record to the configured Salesforce Custom Object.
...
class SalesforceUploaderApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        # ── Title ──────────────────────────────────────────────────────
        tk.Label(
            self, text="Salesforce Uploader", font=("Segoe UI", 14, "bold")
        ).pack(**pad, anchor="w")

        tk.Label(
            self,
            text=f"Target Object: {cfg.SF_OBJECT_API_NAME}  |  Domain: {cfg.SF_DOMAIN}",
        ...
        tk.Entry(file_frame, textvariable=self._file_var, width=52, state="readonly").pack(
            side="left", padx=(0, 6)
        )
        ...
        self._load_btn = tk.Button(
            btn_frame, text="📂  엑셀 불러오기", width=20, command=self._load_excel
        )
        ...
        def _cred_row(parent: tk.Frame, label: str, var: tk.StringVar, show: str = "") -> None:
        ...
        self._upload_btn = tk.Button(
            up_frame, text="⬆  Salesforce 업로드",
            width=22, state="disabled", command=self._upload
        )
        ...
        self._log = scrolledtext.ScrolledText(
            self, height=9, state="disabled", font=("Consolas", 9)
        )
    ...
    def _load_excel(self) -> None:
        ...
        try:
            ...
            preview = preview_records(self._records)
    ...
    def _do_upload(self) -> None:
        def on_progress(done: int, total: int) -> None:
            self.after(0, lambda: setattr(self._progress, "value", done) or
                       self._progress.config(value=done))

        try:
            success, fail, errors = self._connector.insert_records(
                self._records, on_progress=on_progress
            )
            self.after(0, self._on_upload_done, success, fail, errors)
        except Exception as exc:
            self.after(0, self._on_upload_error, str(exc))

    def _on_upload_done(self, success: int, fail: int, errors: list[str]) -> None:
        self._upload_btn.config(state="normal", text="⬆  Salesforce 업로드")
        self._progress["value"] = self._progress["maximum"]
        self._log_write(
            f"[업로드 완료] 성공: {success}건  |  실패: {fail}건\n"
        )
        if errors:
